...
module type ENV = sig
  ...
  module Form : sig
    ...
    val make: ('a, 'b) params_type -> ('a -> 'c Lwt.t) -> (unit -> ('c -> action_on_form) Eliom_lib.client_value) option -> unit -> div_content
    (** a parametrized form is a form whose some fields are hidden *)
    val make_parametrized: ('a, 'b) params_type -> ('c, 'd) params_type ->  ('c -> 'a -> 'e Lwt.t) -> (unit -> ('e -> action_on_form) Eliom_lib.client_value) option -> 'c ->  unit -> div_content
  end
end
